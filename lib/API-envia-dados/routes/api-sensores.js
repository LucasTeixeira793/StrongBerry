...
router.get("/sendData", (request, response) => {
	...
	if (env == "dev") {
		...
		instrucaoSql = `INSERT into tblDados (temperatura, umidade, dataColeta, estufa)
		values (${temperature + (Math.random()*10+1)}, ${Humidity + (Math.random()*20+1)}, '${agora()}', 1),
		(${temperature - (Math.random()*10+1)}, ${Humidity + (Math.random()*20+1)}, '${agora()}', 2),
		(${temperature + (Math.random()*5+1)}, ${Humidity - (Math.random()*20+1)}, '${agora()}', 3),
		(${temperature - (Math.random()*5+1)}, ${Humidity - (Math.random()*20+1)}, '${agora()}', 4);`;
		...
		instrucaoSql = `INSERT into bdstrongberry.tblDados (temperatura, umidade, dataColeta, estufa)
		values (${temperature + (Math.random()*10+1)}, ${Humidity + (Math.random()*20+1)}, '${agora()}', 1),
		(${temperature - (Math.random()*10+1)}, ${Humidity + (Math.random()*20+1)}, '${agora()}', 2),
		(${temperature + (Math.random()*5+1)}, ${Humidity - (Math.random()*20+1)}, '${agora()}', 3),
		(${temperature - (Math.random()*5+1)}, ${Humidity - (Math.random()*20+1)}, '${agora()}', 4);`;
