...
router.get("/tempo-real/:idcaminhao", function (req, res, next) {
    ...
    if (env == "dev") {
        // abaixo, escreva o select de dados para o Workbench
        instrucaoSql = `select temperatura, umidade, DATE_FORMAT(dataColeta,'%H:%i:%s') as momento_grafico, sensores from tblDados where sensores = ${idcaminhao} order by idDados desc limit 1`;
    } else if (env == "production") {
        // abaixo, escreva o select de dados para o SQL Server
